    ...
    <section className="space-y-7">
      <header>
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Nothing here is gone forever. Restore work whenever you need it.
        </p>
      </header>
      {!archivedProjects.length && !archivedTasks.length ? (
        <div className="rounded-2xl border border-dashed bg-card px-6 py-20 text-center shadow-sm">
          <ArchiveRestore className="mx-auto mb-3 size-7 text-primary" />
          <h2 className="font-medium">Nothing archived</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Archived projects and tasks can be restored here.
          </p>
        </div>
      ) : (
            ...
            >
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Archived{" "}
                  {project.archivedAt
                    ? formatDate(project.archivedAt.toISOString().slice(0, 10))
                    : ""}
                </p>
              </div>
