...
import { Archive, ChevronDown, ChevronRight, Flag } from "lucide-react";
...
export function TaskTree({ projectId, tasks }: { projectId: string; tasks: TaskTreeNode[] }) {
  ...
  return <div className="divide-y rounded-lg border bg-background">{tasks.map((task) => <TaskBranch key={task.id} task={task} projectId={projectId} depth={0} />)}</div>;
}

function TaskBranch({ task, projectId, depth }: { task: TaskTreeNode; projectId: string; depth: number }) {
  ...
  return <div><div className="flex min-h-16 items-center gap-2 px-3 py-2" style={{ paddingLeft: `${12 + depth * 24}px` }}>
    ...
    <div className="min-w-0 flex-1"><p className={completed ? "truncate text-sm text-muted-foreground line-through" : "truncate text-sm font-medium"}>{task.title}</p><div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><TaskStatusBadge status={task.status} /><PriorityBadge priority={task.priority} />{task.dueDate && <span>{formatDate(task.dueDate)}</span>}{task.focusDate && <span className="inline-flex items-center gap-1"><Flag className="size-3" />Focused</span>}</div></div>
  ...
  </div>{expanded && task.children.map((child) => <TaskBranch key={child.id} task={child} projectId={projectId} depth={depth + 1} />)}</div>;
}
