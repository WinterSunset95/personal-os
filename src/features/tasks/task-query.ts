...
export const taskSorts = ["priority", "due_date", "status", "updated_at"] as const;
export const dueFilters = ["today", "this_week", "overdue", "no_due_date"] as const;
export type TaskSort = (typeof taskSorts)[number];
export type DueFilter = (typeof dueFilters)[number];
export type TaskQuery = { sort: TaskSort; direction: "asc" | "desc"; statuses: TaskStatus[]; priorities: Priority[]; due: DueFilter | null; hasAttachments: boolean };
...
export function parseTaskQuery(source: URLSearchParams | Record<string, string | string[] | undefined>): TaskQuery {
  ...
  const values = <T extends readonly string[]>(key: string, allowed: T) => (read(key) ?? "").split(",").filter((value): value is T[number] => allowed.includes(value as T[number]));
...
function compareDefault(a: TaskRecord, b: TaskRecord) {
  const aDone = a.status === "completed"; const bDone = b.status === "completed";
  if (aDone !== bDone) return aDone ? 1 : -1;
  if (!aDone && priorityRank[a.priority] !== priorityRank[b.priority]) return priorityRank[a.priority] - priorityRank[b.priority];
...
export function compareTasks(a: TaskRecord, b: TaskRecord, query: TaskQuery = defaultTaskQuery) {
  if (query.sort === "priority" && query.direction === "asc") return compareDefault(a, b);
  let result = 0;
  if (query.sort === "priority") result = priorityRank[a.priority] - priorityRank[b.priority];
  if (query.sort === "status") result = statusRank[a.status] - statusRank[b.status];
  if (query.sort === "due_date") { if (!a.dueDate) result = 1; else if (!b.dueDate) result = -1; else result = a.dueDate.localeCompare(b.dueDate); }
  if (query.sort === "updated_at") result = a.updatedAt.getTime() - b.updatedAt.getTime();
