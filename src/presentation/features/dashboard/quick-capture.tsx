...
export function QuickCapture({ projects, onQuickCaptureTask = defaultQuickCaptureTask }: QuickCaptureProps) {
  const [title, setTitle] = useState(""); const [projectId, setProjectId] = useState(""); const [pending, startTransition] = useTransition();
  const submit = () => { if (!title.trim()) return; startTransition(async () => { await onQuickCaptureTask(title, projectId || undefined); setTitle(""); setProjectId(""); }); };
