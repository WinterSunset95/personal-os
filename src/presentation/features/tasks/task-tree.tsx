...
function TaskBranch({ task, projectId, depth, forceExpanded }: { task: TaskTreeNode; projectId: string; depth: number; forceExpanded: boolean }) {
  ...
  </div>{expanded && task.children.map((child: TaskTreeNode) => <TaskBranch key={child.id} task={child} projectId={projectId} depth={depth + 1} forceExpanded={forceExpanded} />)}</div>;
}
