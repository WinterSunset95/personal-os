            ...
            <label key={tag.id} className="flex cursor-pointer items-center gap-2 rounded px-2 py-1 text-xs hover:bg-muted">
              <Checkbox
                ...
                onCheckedChange={() => {
                  ...
                  start(async () => { await setTaskTags(task.id, projectId, ids as string[]); });
                }}
              />
          ...
          <div className="mt-2 space-y-1 border-t pt-2">
            {task.attachments.map((file: any) => (
              <div key={file.id} className="flex items-center justify-between gap-2 text-xs">
                ...
                <Button size="icon-sm" variant="ghost" className="h-6 w-6 text-destructive hover:text-destructive hover:bg-destructive/10" disabled={pending} onClick={() => start(async () => { await removeTaskAttachment(file.id, projectId); })}>
                  Remove
                </Button>
              </div>
            ))}
