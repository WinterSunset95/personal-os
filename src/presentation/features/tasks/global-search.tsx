  ...
  useEffect(() => {
    const timeout = setTimeout(
      () => {
        ...
        else start(async () => setResults(await searchTasks(term)));
      },
      term.trim() ? 180 : 0,
    );
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-xl">
          ...
          <Input
            ...
            placeholder="Search tasks, projects, tags, or files…"
          />
            ...
            {results.map((result) => (
              <button
                ...
                onClick={() => {
                  setOpen(false);
                  router.push(
                    `/projects/${result.projectId}?task=${result.id}`,
                  );
                }}
                className="flex w-full items-center justify-between gap-3 border-b px-2 py-3 text-left hover:bg-muted"
              >
