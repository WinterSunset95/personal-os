...
import {
  ...
  deleteTaskView,
  updateTaskView,
} from "@/actions/task-view.actions";
import {
  sameTaskQuery,
...
export function SavedViews({
  ...
  const open = (mode: DialogMode) => {
    ...
    setName(
      mode === "rename"
        ? (selected?.name ?? "")
        : `${selected?.name ?? ""}${selected ? " copy" : ""}`,
    );
    setDialogMode(mode);
  };
  const navigate = (view?: TaskViewOption) => {
    const params = view
      ? taskQueryToSearchParams(view.query, view.id)
      : taskQueryToSearchParams(query);
  ...
  const submit = async () => {
    ...
    try {
      if (dialogMode === "rename" && selected && !selected.builtIn) {
        await updateTaskView(selected.id, {
          name,
  ...
  const update = async () => {
    ...
    try {
      ...
      setError(
        cause instanceof Error ? cause.message : "Unable to update view.",
      );
    } finally {
      setBusy(false);
    }
  ...
  const remove = async () => {
    ...
    setError("");
    try {
      await deleteTaskView(selected.id);
      navigate();
      router.refresh();
    } catch (cause) {
      setError(
        cause instanceof Error ? cause.message : "Unable to delete view.",
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="space-y-2 rounded-xl border bg-background p-3">
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex max-w-full gap-1 overflow-x-auto">
          ...
          {views.map((view) => (
            <Button
              ...
              type="button"
              size="sm"
              variant={selected?.id === view.id ? "secondary" : "ghost"}
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onSelect={() => open("save")}>
              <BookmarkPlus />
              Save as new view
            </DropdownMenuItem>
      ...
      <Dialog
        ...
        onOpenChange={(value) => !value && setDialogMode(null)}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {dialogMode === "rename" ? "Rename view" : "Save view"}
            </DialogTitle>
            <DialogDescription>
              {dialogMode === "rename"
                ? "Choose a clear name for this saved view."
                : "Save the current sort and filters for quick access."}
            </DialogDescription>
          </DialogHeader>
          <Input
            autoFocus
